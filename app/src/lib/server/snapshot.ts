...
export function enrichWorkout(w: any, exById: Map<string, any>, tz?: string) {
	...
	const exercises = (w.entries || []).map((e: any) => {
		...
		return {
			...
			// Which version of the movement was performed this session.
...
export function buildSnapshot(s: SnapshotSources) {
	...
	return {
		_readme: [
			...
			'{ name, type, unit, bodyweight, muscles, equipment, perSide, sets, setCount,',
			'volumeKg, note, pain }. Strength sets are { reps, weight }; cardio',
			'{ duration, distance, pace }. volumeKg is null where kilograms are',
			'meaningless: a "sec" unit puts seconds in the weight field, and bodyweight',
			'exercises log no external load.',
			'',
			'One exercise covers every version of a movement: `equipment` ("barbell",',
			'"dumbbell", "cable", "smith"…, null if not recorded) and `perSide` (true =',
			'worked one arm/leg at a time) say which version that session was. Loads are',
			'only comparable within the same version — a dumbbell 40 is not a barbell 40.',
			'',
